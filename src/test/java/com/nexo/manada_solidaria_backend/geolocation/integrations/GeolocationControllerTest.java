...
public class GeolocationControllerTest extends BaseAuthenticatedIntegrationTest {
    ...
    private static final String MOCK_DATA =
            "com.nexo.manada_solidaria_backend.geolocation.utils.MockGeolocationDataUtils#";
    ...
    @MethodSource(MOCK_DATA + "provideResponseFieldCases")
    ...
    @ParameterizedTest(name = "{index} - {0}")
    @MethodSource(MOCK_DATA + "provideProviderErrorCases")
    ...
    void locate_unauthorized_returns401(String testName, String token) throws Exception {
        MockHttpServletRequestBuilder request = get("/geolocation")
                .with(mockRequest -> {
                    mockRequest.setRemoteAddr(CLIENT_IP);
                    return mockRequest;
                });
        if (token != null) {
